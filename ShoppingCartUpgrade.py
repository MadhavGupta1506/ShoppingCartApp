...
def addCart(index):
    ...
    cartItems.append(itemsList[index]) 
...
def removeItem():
    ...
    delete.grid(row=len(itemsList)%9,column=0,sticky=N,padx=10,pady=10,ipadx=30,ipady=20)
    deleteCancel.grid(row=len(itemsList)%9,column=1,sticky=N,padx=10,pady=10,ipadx=30,ipady=20)
...
def submitButton():
    ...
    if(response==True):
        ...
        if(len(cartItems)==0):
            totalLabel=Label(top,text=sum,width=100,height=20,bd=5)
            totalLabel.pack()
        else:
            #If there are items in the cart this loop label them on the toplevel with their total
            for i in range(len(cartItems)):
                itemLabel=Label(top,text=f"{cartItems[i]}",font= ("Courier New",15,"bold"),padx=30,width=5)
                itemPrice=Label(top,text=f"{items[cartItems[i]]}",font=("Courier New",15,"bold"))
                itemLabel.grid(row=i,column=0)
                itemPrice.grid(row=i,column=1)
            totalLabel=Label(top,text=sum,font=("Comic Sans MS", 25, "bold"),width=100,height=20,bd=5)
...
def showCart():
    ...
    cartDisplay=Label(text=itemNames,padx=4)
...
submit.grid(row=len(itemsList)%9,column=5,padx=10,pady=10,ipadx=30,ipady=20)
...
itemsInCart.grid(row=len(itemsList)%9,column=3,sticky=N,padx=10,pady=10,ipadx=30,ipady=20)
